...
import { FaCalendar, FaClock, FaDollarSign, FaMoneyBillAlt, FaStar } from "react-icons/fa";
...
export function MovieInfoCard({ logo, title, value } : MovieInfoCardProps){
  ...
  return(
    <div className="w-[8rem] h-[8rem] flex flex-col items-center justify-center rounded-lg inset-0 bg-[#ffffff10] backdrop-blur-md text-center">
      {logo === 'calendar' && <FaCalendar size={30} color={colors.blue['500']}/>}
      {logo === 'clock' && <FaClock size={30} color="#FFF"/> }
      {logo === 'dollar' && <FaDollarSign size={30} color={colors.green['500']}/>}
      {logo === 'money' && <FaMoneyBillAlt size={30} color={colors.green['500']}/>}
